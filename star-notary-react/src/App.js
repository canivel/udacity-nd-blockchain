...
function App() {
  ...
  useEffect(async () => {
    ...
    setMeta(
      new web3.eth.Contract(starNotaryArtifact.abi, deployedNetwork.address) // Initializing the contract
    );
  ...
  async function starNameFunc() {
    const { starName } = meta.methods; // to be able to use the functions in your Smart Contract use destructuring to get the function to be call
  ...
  async function starOwnerFunc() {
    const { starOwner } = meta.methods; // to be able to use the functions in your Smart Contract use destructuring to get the function to be call
  ...
  async function claimStarFunc() {
    const { claimStar, starOwner } = meta.methods; // to be able to use the functions in your Smart Contract use destructuring to get the function to be call
    await claimStar().send({ from: account }); // Use `send` instead of `call` when you called a function in your Smart Contract
